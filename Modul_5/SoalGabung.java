...
//--------------------ALGORITMA RABIN KARP-----------------------//
	static void RabinKarp(String pattern, String txt) {
	    ...
	    boolean found = false;           // Berguna Untuk Menentukan Jika Tidak Ada Pattern Yang Cocok Pada Text
	    ...
	    for (i = 0; i < pat_long - 1; i++) {
	        h = (h * ASCII) % prime;
	        System.out.println(" Nilai modular h : "+h);
	    }System.out.println();
	    
	    // Hitung Nilai Hash Untuk Pattern And Text
	    for (i = 0; i < pat_long; i++) {
	        hash_p = (ASCII * hash_p + pattern.charAt(i)) % prime;
	        System.out.println(" Nilai hash_p : "+hash_p+" pada perulangan ke-("+i+")");
	        hash_t = (ASCII * hash_t + txt.charAt(i)) % prime;
	        System.out.println(" Nilai hash_t : "+hash_t+" pada perulangan ke-("+i+")");
	    }System.out.println();
	    ...
	    for (i = 0; i <= txt_long - pat_long; i++) {
	        ...
	        if (i < txt_long - pat_long) {
	            ...
	            System.out.println(" Loop ["+(i+1)+"] nilai hash_t i : "+hash_t);
	            if (hash_t < 0) {
	                hash_t = (hash_t + prime);
	                System.out.println(" Loop ["+(i+1)+"] nilai hash_t < 0 = "+hash_t);
	            }
	        }
	    }System.out.println();
	    
	    if (!found) {
	        System.out.println(" Tidak ada pattern yang cocok dengan teks.");
	    }
	    }
	}
